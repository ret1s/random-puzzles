...
impl Kid {
    pub fn parse_row(csv_row: &str) -> Result<Kid, &'static str> {
        ...
        let good_deeds = parts[1].trim().parse::<u32>().map_err(|_| "Invalid number for good deeds")?;
        let bad_deeds = parts[2].trim().parse::<u32>().map_err(|_| "Invalid number for bad deeds")?;
