...
def parse_input():
    ...
    # Shapes are all sections except the last.
    shape_areas = []
    ...
    regions = sections[-1].strip().splitlines()
    return shape_areas, regions


def solution():
    shape_areas, regions = parse_input()
    ...
    for line in regions:
        ...
        required_area = sum(c * a for c, a in zip(counts, shape_areas))
