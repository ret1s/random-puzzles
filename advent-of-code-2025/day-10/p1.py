...
def parse_line(line: str) -> Tuple[List[int], List[List[int]]]:
    ...
    # Extract buttons (parentheses groups before the curly brace).
    buttons = []
    ...
    if paren_parts:
        for part in paren_parts:
            ...
            if not part:
                continue
            indices = [int(x) for x in part.split(",") if x != ""]
            buttons.append(indices)

    return target, buttons


def rref_solve_min_weight(target: List[int], buttons: List[List[int]]) -> int:
    m = len(buttons)
    n = len(target)
    if m == 0:
        # No buttons; only solvable if target all zeros.
        return 0 if all(v == 0 for v in target) else float("inf")

    total_cols = m
    rhs_bit = 1 << total_cols
    row_mask = (1 << total_cols) - 1

    # Build matrix rows with RHS.
    rows = []
    for light_idx, t in enumerate(target):
        mask = 0
        for col, idx_list in enumerate(buttons):
            if light_idx in idx_list:
                mask |= 1 << col
        row = mask | (t * rhs_bit)
        rows.append(row)
    ...
    # Check consistency.
    for row in rows:
        if (row & row_mask) == 0 and (row & rhs_bit):
            return float("inf")  # No solution; shouldn't happen per problem.

    pivot_set = set(pivot_cols)
    free_cols = [c for c in range(total_cols) if c not in pivot_set]

    # Particular solution with free variables = 0.
    particular = 0
    for prow, pcol in zip(pivot_rows, pivot_cols):
        rhs = (rows[prow] >> total_cols) & 1
        if rhs:
            particular |= 1 << pcol

    # Nullspace basis for each free variable.
    basis = []
    for f in free_cols:
        vec = 1 << f
        for prow, pcol in zip(pivot_rows, pivot_cols):
    ...
    # Enumerate all combinations of free variables to find min Hamming weight.
    k = len(basis)
...
def solution():
    ...
    for line in lines:
        ...
        presses = rref_solve_min_weight(target, buttons)
